...
export class LeaderboardDisplay {
    ...
    createTournamentInfo(data) {
        ...
        // Add tournament phase if available
        const tournamentPhase = this.formatTournamentPhase(data);
        if (tournamentPhase) {
            tournamentInfo.appendChild(tournamentPhase);
        }
    ...
    createLeaderboardTable(data) {
        ...
        // Create rows using DocumentFragment for better performance
    ...
    createStats(data) {
        ...
        totalPlayers.textContent = `Total Players: ${data.total_players}`;
        
        const lastUpdated = document.createElement('div');
        lastUpdated.className = 'last-updated';
        lastUpdated.textContent = `Updated: ${new Date(data.updated_at).toLocaleTimeString()}`;
    ...
    addEventListeners() {
        ...
        document.getElementById('closeLeaderboardBtn').addEventListener('click', () => {
            // console.log('🏆 Leaderboard close button clicked - returning to game context');
            this.hide();
        });
    ...
    createLeaderboardRow(entry, index) {
        ...
        // Handle both display_name and player_name fields for backward compatibility
        const playerName = entry.display_name || entry.player_name || 'Unknown Player';
        
        // Use rank from backend if available, otherwise calculate from index
        const rank = entry.rank || (index + 1);
        
        // Format the timestamp in EST
        const scoreTime = this.formatScoreTime(entry.timestamp);
        
        // Debug: Check if player_id is an object
        let playerId = entry.player_id;
        if (typeof playerId === 'object') {
            console.warn('⚠️ Player ID is an object:', playerId);
        ...
        rankCell.innerHTML = `${rankIcon} ${rank}`;
        ...
        playerCell.appendChild(playerNameDiv);
        
        const playerIdDiv = document.createElement('div');
        playerIdDiv.className = 'player-id';
        playerIdDiv.textContent = playerId;
        playerCell.appendChild(playerIdDiv);
    ...
    formatResetTime(isoString, formattedDate) {
        // Use the pre-formatted date from backend if available
        if (formattedDate) {
            return `11:15 PM EST on ${formattedDate}`;
        }
        
        // Fallback to current date if backend doesn't provide formatted date
    ...
    formatScoreTime(timestamp) {
        ...
        try {
            // Convert timestamp to EST/EDT (Eastern Time)
            ...
            // Use browser's timezone handling for Eastern Time (automatically handles DST)
            ...
            console.error('Error formatting score time:', error);
            return '--:--';
        }
    }
    ...
    formatTournamentPhase(data) {
        ...
        switch (data.tournament_phase) {
            ...
            case 'active':
                ...
                tournamentStatus.appendChild(statusIcon);
                tournamentStatus.appendChild(statusText);
